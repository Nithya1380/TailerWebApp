...
(function withAngular(angular, navigator) {
  ...
  var A_DAY_IN_MILLISECONDS = 86400000
    ...
    , generateDays = function generateDays() {
      ...
      return [
        '<div class="_720kb-datepicker-calendar-body">',
          '<a href="javascript:void(0)" ng-repeat="px in prevMonthDays" ng-click="setDatepickerDay(px, -1)" class="_720kb-datepicker-calendar-day _720kb-datepicker">',
            '{{px}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="item in days" ng-click="setDatepickerDay(item, 0)" ng-class="{\'_720kb-datepicker-today\': item === today.getDate() && monthNumber === (today.getMonth() + 1) && year === today.getFullYear() && !selectedDay,\'_720kb-datepicker-active\': selectedDay === item && selectedMonth === monthNumber && selectedYear === year}" class="_720kb-datepicker-calendar-day">',
            '{{item}}',
          '</a>',
          '<a href="javascript:void(0)" ng-repeat="nx in nextMonthDays" ng-click="setDatepickerDay(nx, 1)" class="_720kb-datepicker-calendar-day _720kb-datepicker">',
            '{{nx}}',
          '</a>',
        '</div>'
      ];
    },
     generatebottomColumns = function generatebottomColumns() {

         return [
         '<div class="_720kb-datepicker-calendar-header">',            
            '<div class="_720kb-datepicker-calendar-header-center _720kb-datepicker-calendar-month ng-binding">',
            ' <a ng-click="FocusToday()" class="alert alert-default" style="color:#2e6e9e; background-color:white; margin-right:110px" >Today</a>' +
                ' <a ng-click="hideCalendar()" class="alert alert-default" style="color:#2e6e9e; background-color:white; margin-right:1px" > Close</a>',
            '</div>',
         '</div>'
         ];
     }

     //_720kb-datepicker-calendar-header-middle _720kb-datepicker-calendar-month ng-binding
    ...
    , generateHtmlTemplate = function generateHtmlTemplate(prevButton, nextButton, preventMobile) {
      ...
      , days = generateDays()
      , PageBottom = generatebottomColumns()
    ...
    , datepickerDirective = function datepickerDirective($window, $compile, $locale, $filter, $interpolate, $timeout) {
      ...
      var linkingFunction = function linkingFunction($scope, element, attr) {
        ...
        var selector = attr.selector
          ...
          , theCalendar
          , defaultPrevButton = '<b class="fa fa-arrow-circle-left"></b>'
          , defaultNextButton = '<b class="fa fa-arrow-circle-right"></b>'
          ...
          , setInputValue = function setInputValue(IsOutside) {

            if ($scope.isSelectableMinDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day) &&
                $scope.isSelectableMaxDate($scope.year + '/' + $scope.monthNumber + '/' + $scope.day)) {

                var MOnth = parseInt($scope.monthNumber) + parseInt(IsOutside);
                if (MOnth > 12) {
                    $scope.year += 1;
                    MOnth = 1;
                }
                ...
                var modelDate = new Date($scope.year + '/' + MOnth + '/' + $scope.day);
        ...
        $scope.FocusToday = function hideCalendar() {
            var myDate = new Date();
            $scope.monthNumber = myDate.getMonth() + 1;
            $scope.year = myDate.getFullYear();
          
            //check if max date is ok
            if ($scope.dateMaxLimit) {
                if (!$scope.isSelectableMaxDate($scope.year + '/' + $scope.monthNumber + '/' + myDate.getDate())) {
                    resetToMaxDate();
                }
            } 
            //set next month
            ...
            $scope.setDatepickerDay($scope.day, 2);
        ...
        $scope.setDatepickerDay = function setDatepickerDay(day, IsOutside) {
